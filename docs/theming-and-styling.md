...
`@endeavoury/kanosis-tokens/tokens.css` defines primitive scales and semantic aliases. Components consume semantic variables such as `--ds-color-bg-surface`, never theme-specific raw colors. Light and dark themes assign the same semantic names.
...
Importing `@endeavoury/kanosis/styles.css` is optional (the same file is also available from `@endeavoury/kanosis-styles/global.css`). It affects:
